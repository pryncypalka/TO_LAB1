...
class TkApp:
    def __init__(self, root, collection, exchange):
        ...
        self._currencies_dict = self._collection.get_dict()
        self._currency_names = list(self._currencies_dict.keys())
        self._exchange = exchange
        self._root = root

        self._root.title("Kalkulator Walutowy")
        self.label_currency1 = ttk.Label(root, text="Waluta 1:")
        self.label_currency1.grid(row=0, column=0)
        self.currency1 = ttk.Combobox(root, values=self._currency_names)
        ...
        self.currency2 = ttk.Combobox(root, values=self._currency_names)
    ...
    def update_conversion(self, event):
        try:
            ...
            if amount1_str and float(amount1_str) > 0:  # Sprawdź, czy wpisano liczbę dodatnią
                ...
                self._exchange.set_W1(self._currencies_dict[currency1])
                self._exchange.set_W2(self._currencies_dict[currency2])
                self._exchange.set_amount(amount1)

                self.result.config(text=f"{amount1_str} {currency1} = {self._exchange.result()} {currency2}")
            else:
